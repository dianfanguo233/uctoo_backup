...
class ToolController extends HomeController {
	...
	function upateModel() {
		// 先判断模型
		$this->adddel ( 'uctoo_model', 'name' );
		
		// 属性更新
		$table = 'uctoo_attribute';
		$sql = "SELECT a.*, m.name as model_name FROM {$this->db1}.`$table` a left join {$this->db1}.uctoo_model m on a.model_id=m.id";
		$list1 = M ()->query ( $sql );
		...
		$sql = "SELECT a.*, m.name as model_name FROM {$this->db2}.`$table` a left join {$this->db2}.uctoo_model m on a.model_id=m.id";
		$list2 = M ()->query ( $sql );
