...
class TestController extends HomeController {
	function attr2() {
		...
		foreach ( $model_list as $m ) {
			$mod [$m ['id']] = 'uctoo_' . $m ['name'];
		}
		
		$model_list = M ( 'model_copy' )->field ( 'id,name' )->select ();
		foreach ( $model_list as $m ) {
			$mod2 [$m ['id']] = 'uctoo_' . $m ['name'];
		}
...
// 		dump ( $att2 );
	...
	function mylove() {
		// ALTER TABLE `uctoo_prize` ADD COLUMN `token` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT 'Token' AFTER `img`;
		// ALTER TABLE `uctoo_sn_code` ADD COLUMN `token` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT 'Token' AFTER `prize_title`;
		$_token = array ();
		$dao = M ( 'prize' );
		$list = $dao->select ();
		foreach ( $list as $v ) {
