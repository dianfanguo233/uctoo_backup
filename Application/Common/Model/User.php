...
class User
{
    private $table_fields = array(
        ...
        //ucmember
        'id', 'username', 'password', 'email', 'mobile', 'reg_time'
    );
    ...
    function query_user($pFields = null, $uid = 0)
    {
        ...
        if (array_intersect(array('score','score1'), (array)$pFields)) {
        ...
        if (in_array('is_followed', $fields)) {
            $follow = db('Follow')->where(array('who_follow' => $uid, 'follow_who' => get_uid()))->find();
    ...
    private function getAlias($uid)
    {
        ...
        if ($alias === false) {
            //没有缓存
            $alias = '';
            $follow = db('Common/Follow')->getFollow(get_uid(), $uid);//获取关注情况
...
//查询缓存，过滤掉已缓存的字段
    ...
    {
//获取两张用户表格中的所有字段
        $homeFields = db('Member')->getDBFields();
        $ucenterFields = db('UcenterMember')->getDBFields();
        ...
        $ucenterResult = array();
        if ($homeFields) {
            $homeResult = db('Member')->where(array('uid' => $uid))->field($homeFields)->find();
        }
        if ($ucenterFields) {
            $model = db('UcenterMember');
...
//获取个人中心地址
        ...
        if (array_intersect(array('space_url', 'space_link', 'space_mob_url'), $fields)) {
            ...
            $urls['space_mob_url'] = url('Mob/User/index', array('uid' => $uid));
...
//读取用户名拼音
    ...
    public function getRankLink($fields, $uid, $result)
    {
        ...
        if (in_array('rank_link', $fields)) {
            $rank_List = db('rank_user')->where(array('uid' => $uid, 'status' => 1))->select();
            $num = 0;
            foreach ($rank_List as &$val) {
                $rank = db('rank')->where('id=' . $val['rank_id'])->find();
                $val['title'] = $rank['title'];
                $val['logo_url'] = get_pic_src(db('picture')->where('id=' . $rank['logo'])->field('path')->value('path'));
