...
class ModelController extends AdminController {
	...
	public function export() {
		...
		if ($type == 1) {
			$sql = "DELETE FROM `uctoo_attribute` WHERE model_id = (SELECT id FROM uctoo_model WHERE `name`='{$model['name']}' ORDER BY id DESC LIMIT 1);\r\n";
			$sql .= "DELETE FROM `uctoo_model` WHERE `name`='{$model['name']}' ORDER BY id DESC LIMIT 1;\r\n";
			$sql .= "DROP TABLE IF EXISTS `uctoo_" . strtolower ( $name ) . "`;";
			...
			$sql .= <<<sql
CREATE TABLE IF NOT EXISTS `uctoo_{$name}` (
{$create_table}{$key}
...
sql;
			...
			foreach ( $data as $d ) {
				...
				$sql .= "INSERT INTO `uctoo_{$name}` (" . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ");\r\n";
			}
			...
			$sql .= 'INSERT INTO `uctoo_model` (' . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ');' . "\r\n";
			...
			foreach ( $list as $k => $vo ) {
				...
				$sql .= 'INSERT INTO `uctoo_attribute` (' . rtrim ( $field, ',' ) . ') VALUES (' . rtrim ( $value, ',' ) . ');' . "\r\n";
			}
			$sql .= 'UPDATE `uctoo_attribute` SET model_id= (SELECT MAX(id) FROM `uctoo_model`) WHERE model_id=0;';
