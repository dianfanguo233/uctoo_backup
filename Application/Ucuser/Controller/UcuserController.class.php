...
class UcuserController extends AdminController
{
    ...
    {
        $model = D('Ucuser');
		$show_type = I('show_type',0,'int');
        $map['status'] = array('EGT', 0);
        $map['mp_id'] = get_mpid();
		empty($_GET['taglist']) || $taglist = I('taglist/a','','intval');
		if(!empty($taglist))
		{
...
//		var_dump(__file__.' line:'.__line__,$model->getLastSql());exit;
	...
	public function editUcuserTagLink()
	{
		...
		else
		{
			...
			$this->assign('posturl',U('admin/ucuser/editUcuserTagLink'));
...
//					$user_info['tagid'.$i.'name'] = $UcTag->where(array('id'=>$user_info['tagid_list'][$i]))->getfield('name');
    ...
    public function sycUcuser(){
		...
		($temp_id = I('temp_id',false,'/\w{13}/')) && $arr =  session($temp_id);
		...
		if($res['count'] < $res['total'])
		{
			...
			$this->success('正在获取粉丝列表('.$res['count'].'/'.$res['total'].')，请不要关闭页面',
				U('admin/ucuser/sycucuser/temp_id/'.$temp_id));
		}
		else
		{
			$this->success('成功获取粉丝列表，数量：'.$res['total'].'个',U('index'));
		}
    }
...
//		var_dump(__file__.' line:'.__line__,$ret);exit;
		...
		$builder
			->title('用户列表')
			->button('同步微信标签列表', array('href' => U('/admin/Ucuser/sycUcuserTag')))
			->buttonModalPopup(U('/admin/Ucuser/createUcuserTags'),'','新建微信标签')
			->keyText('id', 'id')
			->keyLinkByFlag('name', '分组名','/admin/ucuser/index/show_type/1&taglist%5B%5D=###&','id')
			->keyText('count', '数量')
			->keyDoActionModalPopup('/admin/ucuser/updateUcuserTag/id/###', '重命名','操作')
			->keyDoActionModalPopup('/admin/ucuser/deleteUcusersTag/id/###', '删除','操作')
			->data($ret['list'])
			->pagination($totalCount, $r)
			->display();
	}
	...
	public function deleteUcusersTag()
	{
		...
		if(IS_POST)
		{
			...
			$this->assign('posturl',U('admin/ucuser/deleteUcusersTag'));
			$this->display('Public/popup');
		}
	}
	/*
	...
	public function updateUserRemark()
	{
		...
		else
		{
			...
			$this->assign('inputs',$inputs);
			$this->assign('data',array('ids'=>$ids));
			$this->assign('posturl',U('admin/ucuser/updateUserRemark'));
			$this->display('Public/popup');
		}
	}
	/*
	...
	public function updateUcuserTag()
	{
		...
		if(IS_POST)
		{
			if(in_array($id,array('0','1','2')))
			{
				$this->error('这个标签默认保留的标签,不能修改',U('admin/ucuser/ucuser_tag'));
			}
			$name = I('name','','text');
			$map['id'] = $id;
			$tagid = $UcTag->where($map)->find();
		...
		else
		{
			...
			$this->assign('inputs',$inputs);
			$this->assign('data',array('id'=>$id));
			$this->assign('posturl',U('admin/ucuser/updateUcuserTag'));
			$this->display('Public/popup');
		}
	}
	public function createUcuserTags()
	{
		...
		else
		{
			...
			$this->assign('posturl',U('admin/ucuser/createUcuserTags'));
			$this->display('Public/popup');
		}
	}
	/*
