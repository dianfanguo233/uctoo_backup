...
class UploadAvatar extends Controller
{
    ...
    public function render($uid = 0)
    {
        ...
        $this->assign('uid', $uid);
        $this->fetch('Application://Ucenter@Widget/uploadavatar');
    }
    ...
    public function getAvatar($uid = 0, $size = 256)
    {
        ...
        if ($avatar) {
           ...
           /* if($uid==session('temp_login_uid')||$uid==is_login()){
            ...
            $role_id=db('member')->where('uid='.$uid)->field('show_role')->select();
    ...
    private function getImageUrlByRoleId($role_id, $size)
    {
        ...
        if(!$avatar_id){
            ...
            $avatar_id = model('admin/RoleConfig')->where($map)->field('value')->find();
        ...
        if ($avatar_id) {
            ...
            if ($size != 0) {
                ...
                //$path=$default_avatar;
                trace($path.'___size','info');
            } else {

                $default_avatar = "images/default_avatar.jpg";
                $path= get_pic_src("\images\default_avatar.jpg");
               // $path=$default_avatar;
                trace($path.'___else','info');
            }
        }
        return $path;
    }
