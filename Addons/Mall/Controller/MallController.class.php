...
class MallController extends AddonsController{	
	...
	public function cpinfo(){
		...
		//商品属性
		$attrs_tmp=M()->query("select a.id,a.aid,a.value,a.price,a.desc,b.name,(select path from ".PREX."picture where a.icon=".PREX."picture.id) as path,(select path from ".PREX."picture where a.thumbnail=".PREX."picture.id) as thumbnail from ".PREX."ml_mall_attr_value a,".PREX."ml_mall_attr b where a.gid=".$id." and a.aid=b.id and b.token='".get_token()."'");
    ...
    public function brands($token=null) {
        // $issues = M()->query("select * from ".PREX."issue where pid!=0");
        $branddata = M(ml_mall_set);
        ...
        $brands = $branddata->where($map)->select();

        $brandstype = $branddata->distinct(true)->field('settype')->where($map)->order ( 'settype asc' )->select();
        $modelid = $this->model ['id'];
        $this->assign('brands',$brands);
        $this->assign('modelid',$modelid);
        $this->assign('brandstype',$brandstype);
        ...
        $this->display ( T ( 'Addons://Mall@default/Template/'.$TemplateIndex.'/brands' ) );
    }
	...
	public function lists() {
        ...
        $normal_tips = '“微商城”可以添加多个分店，在微信里回复商店名访问商店。或者通过地址访问品牌首页：' . addons_url ( 'Mall://Mall/brands', $param ) . ' ，也可点击<a target="_blank" href="' . addons_url ( 'Mall://Mall/brands', $param ) . '">这里</a>在线预览';
