...
class DiscountTypeController extends BaseController{	
	...
	public function __construct() {
		parent::__construct ();
		$this->model = M ( 'Model' )->getByName ( 'ml_mall_discount_type' );
	...
	public function yhset(){
		...
		// 使用提示
		$normal_tips = '操作说明:<br/>&nbsp;&nbsp;1.选择您需要配置的商城<br/>&nbsp;&nbsp;2.左边为商品列表,右边优惠已有的商品列表,蓝色部分为优惠类别<br/>&nbsp;&nbsp;3.配置好后保存即可';
		...
		$ctdata = M ("ml_mall_set")->where ($cwhere)->order ( 'id asc' )->select ();
		...
		//查询优惠信息
		$data = M ( "ml_mall_discount_type" )->field ('id,name,setid,dishesdatas')->where ( $map )->order ( 'id DESC' )->select ();
		$newdata = array();
		foreach($data as $yh){
			//还原数据			
		...
		//查询商品
		$dishesdatas = M ("ml_mall_dishes")->field("id,`name`,dishes_num,introduction,paixu")->where ($cwhere)->order ( 'paixu desc' )->select();
		$this->assign ( 'dishesdatas', json_encode($dishesdatas));
		
		$this->display ( T ( 'Addons://Mall@default/DiscountType/yhset' ) );		
	}
	...
	public function saveyh(){
		...
		foreach($yhdatas as $yh){			
			 M ("ml_mall_discount_type")->where(array("token"=>$token,"setid"=>$ctid,"id"=>$yh["cxid"]))->save(array("dishesdatas"=>serialize($yh["datas"])));
			 //更新商品
			 M ("ml_mall_dishes")->where(array("token"=>$token,"id"=>array("in",implode(",",$yh["datas"]))))->save(array("youhuiid"=>$yh["cxid"]));
		}
		echo "优惠商品配置成功!";
	}
	...
	public function lists() {
		...
		$ctid = I("get.ctid",0, 'intval' );
		// 使用提示
		$normal_tips = '每个商城的优惠单独设置，优惠可以从已有的商城优惠复制添加!<br/>&nbsp;&nbsp;将分类设置为主要后,该优惠分类将出现在商品推荐列表(该功能视主题而定)<br/>&nbsp;&nbsp;只能设置一个优惠分类为主要';
		$this->assign ( 'normal_tips', $normal_tips );
		...
		$this->display ( T ( 'Addons://Mall@default/DiscountType/lists' ) );		
	}
	...
	public function setzhuyao(){
		...
		$id = I ( 'id', 0, 'intval' );	
		$ctid = I("get.ctid",0, 'intval' );
		M ("ml_mall_discount_type")->where (array("token"=>$token,"ctid"=>$ctid))->save(array("ismain"=>0));
		M ("ml_mall_discount_type")->where (array("token"=>$token,"id"=>$id,"ctid"=>$ctid))->save(array("ismain"=>1));		
	...
	public function ctfz(){
		...
		$token = get_token();
		//复制优惠
		$DiscountTypedata_by_from = M ("ml_mall_discount_type")->where (array("token"=>$token,"setid"=>$from))->order ( 'id asc' )->select();
		//不能自己复制自己
		if($from != $to){
			$newDiscountType = M ("ml_mall_discount_type");
