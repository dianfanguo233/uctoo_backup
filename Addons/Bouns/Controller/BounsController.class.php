...
namespace Addons\Bouns\Controller;

use Home\Controller\AddonsController;
use Com\JsSdk;

class BounsController extends AddonsController {
	function edit() {
		$id = I ( 'id' );
		$model = $this->getModel ();
	...
	function set_sn_code() {
		...
		if ($res) {
			// 更新获取数
			M ( "bouns" )->where ( 'id=' . $id )->setInc ( "collect_count" );
			
			// 扣除积分
			if (! empty ( $data ['credit_bug'] )) {
				$credit ['score'] = $data ['credit_bug'];
				$credit ['experience'] = 0;
				add_credit ( 'bouns_credit_bug', 5, $credit );
			}
