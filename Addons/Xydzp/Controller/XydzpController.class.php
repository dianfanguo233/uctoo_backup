...
class XydzpController extends AddonsController
{
    ...
    public function zjloglists()
    {
		...
		$sqlqu = M('xydzp_log')
					...
					->where(array('xydzp_id' => $xydzp_id,$fix . 'ucenter_member.token' => get_token() ));
    ...
    function join()
    {
		...
		if($this->_is_starttime($xydzp_id)){
			if($this->_is_overtime($xydzp_id)){
				...
				$fix      = C("DB_PREFIX");
				//查询奖品列表		
				$jplist   = M('xydzp_jplist')->join('left join ' . $fix . 'xydzp_option on ' . $fix . 'xydzp_jplist.xydzp_option_id=' . $fix . 'xydzp_option.id')->field($fix . "xydzp_option.title,gailv,xydzp_id,xydzp_option_id,". $fix . "xydzp_option.isdf," . $fix . "xydzp_option.num as kcnum," . $fix . "xydzp_option.miaoshu")->where("xydzp_id=$xydzp_id")->order('type asc,gailv asc')->select();
				//按奖品等级计算概率
