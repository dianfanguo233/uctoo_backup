...
class ConfigController extends AddonsController
{
    ...
    public function index() {
        ...
        $ucuser = M('UcenterMember')->where ( $map )->find ();
        ...
        if (IS_POST) {
            ...
            if (! empty ( $_POST ['mobile'] ) && $_POST ['mobile'] != $user ['mobile']) {
                $ucdata ['mobile'] = $data ['mobile'] = I ( 'post.mobile' );
            }
            if (! empty ( $_POST ['sex'] ) && $_POST ['sex'] != $user ['sex']) {
                $data ['sex'] = I ( 'post.sex' );
            }
            if (! empty ( $_POST ['username'] ) && $_POST ['username'] != $user ['username']) {
                $ucdata ['username'] = I ( 'post.username' );
            }
            if (! empty ( $_POST ['email'] ) && $_POST ['email'] != $user ['email']) {
                $ucdata ['email'] = I ( 'post.email' );
            }
            if (! empty ( $data )) {
                $res = M ( 'follow' )->where ( $map )->save ( $data );
            }
            if (! empty ( $ucdata )) {
                $res = M ( 'UcenterMember' )->where ( $map )->save ( $ucdata );
            }


            redirect (  addons_url ( 'UserCenter://Config/index'  ) );
            exit ();
